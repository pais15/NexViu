...
@app.on_message(exists_filter & move_filter('support') & ~filters.text_filter("🏠 خانه"))
...
@app.on_message(exists_filter & filters.text_filter("💜 درباره NexViu"))
async def about_nexviu(client, m: Message):
    await m.reply(HI_MEMBER)


@app.on_message(exists_filter & filters.text_filter("🤝 همکاری با ما"))
