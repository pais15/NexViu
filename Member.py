...
@app.on_message(filters.text_filter("📢 کانال‌ها و گروه‌های من"))
async def my_channels(client, m: Message):
    pass


@app.on_message(filters.text_filter("🚀 ثبت تبلیغ جدید"))
async def new_advertisement(client, m: Message):
    pass


@app.on_message(filters.text_filter("💸 نمایش تبلیغ و کسب درآمد"))
async def show_advertisement(client, m: Message):
    pass


@app.on_message(filters.text_filter("💰 کیف پول و تراکنش‌ها"))
async def wallet_and_transactions(client, m: Message):
    pass


@app.on_message(filters.text_filter("🆘 پشتیبانی و راهنما"))
